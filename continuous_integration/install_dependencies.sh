...
if [[ "$PYTHON_VERSION" == "3.6" ]]; then
  python -m pip install scipy==1.1 numpy==1.15 mkl pip pytest lapack ecos scs osqp flake8 cvxopt
  ...
  python -m pip install scipy==1.3 numpy==1.16 mkl pip pytest lapack ecos scs osqp flake8 cvxopt
elif [[ "$PYTHON_VERSION" == "3.9" ]]; then
  # The earliest version of numpy that works is 1.19.
  # Given numpy 1.19, the earliest version of scipy we can use is 1.5.
  python -m pip install scipy==1.5 numpy==1.19 mkl pip pytest lapack ecos scs osqp flake8 cvxopt
fi
