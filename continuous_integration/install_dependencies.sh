...
if [[ "$PYTHON_VERSION" == "3.6" ]]; then
  conda install scipy=1.1 numpy=1.15 0.3 pip pytest lapack ecos scs osqp flake8 cvxopt
elif [[ "$PYTHON_VERSION" == "3.7" ]]; then
  conda install scipy=1.1 numpy=1.15 pip pytest lapack ecos scs osqp flake8 cvxopt
  ...
  conda install scipy=1.3 numpy=1.16 pip pytest lapack ecos scs osqp flake8 cvxopt
elif [[ "$PYTHON_VERSION" == "3.9" ]]; then
  # The earliest version of numpy that works is 1.19.
  # Given numpy 1.19, the earliest version of scipy we can use is 1.5.
  conda install scipy=1.5 numpy=1.19 pip pytest lapack ecos scs osqp flake8 cvxopt
fi
